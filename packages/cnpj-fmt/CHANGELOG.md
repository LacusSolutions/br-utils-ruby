...
- **Structured errors**: `CnpjFmt::Error` marker with `TypeMismatchError`, `DomainError` / `InvalidLengthError` / `OutOfRangeError`, and `ValidationError`; invalid length is passed to configurable `on_fail` as `InvalidLengthError`.
