...
RSpec.describe CpfDV::TypeMismatchError do
  ...
  context 'when instantiated' do
    ...
    it 'includes CpfDV::Error' do
      expect(error).to be_a(CpfDV::Error)
    end

    it 'exposes the class name' do
      expect(error.class.name).to eq('CpfDV::TypeMismatchError')
    end
...
RSpec.describe CpfDV::DomainError do
  before do
    stub_const('CpfDV::TestDomainError', Class.new(described_class))
  end

  subject(:error) { CpfDV::TestDomainError.new('some error') }
  ...
  context 'when instantiated through a subclass' do
    ...
    it 'exposes the subclass name' do
      expect(error.class.name).to eq('CpfDV::TestDomainError')
    end
...
RSpec.describe CpfDV::InvalidLengthError do
  subject(:error) do
  ...
  context 'when instantiated' do
    ...
    it 'exposes the class name' do
      expect(error.class.name).to eq('CpfDV::InvalidLengthError')
    end
    ...
    it 'builds a descriptive message' do
      expect(error.message).to eq(
        'CPF input "1.2.3.4.5" does not contain 12 to 14 digits. Got 5 in "12345".'
      )
    end
  end
end

RSpec.describe CpfDV::ValidationError do
  subject(:error) do
  ...
  context 'when instantiated' do
    ...
    it 'exposes the class name' do
      expect(error.class.name).to eq('CpfDV::ValidationError')
    end
    ...
    it 'builds a descriptive message' do
      expect(error.message).to eq(
        'CPF input "1.2.3.4.5" is invalid. repeated digits'
      )
    end
  end
end
