...
# Deliberately independent of CpfFormatterOptions::DISALLOWED_KEY_CHARACTERS so
# production changes fail loudly.
DISALLOWED_KEY_CHARACTERS = %w[å ë ï ö].freeze

RSpec.describe CpfFmt::CpfFormatterOptions do
  ...
  def forbidden_key_message(option_name, value)
    ...
    %(Value "#{value}" for CPF formatting option "#{option_name}" contains disallowed characters (#{quoted}).)
  end
  ...
  describe '#initialize' do
    ...
    context 'when called with some parameters' do
      ...
      it 'preserves defaults for mixed nil and valid values' do
        on_fail = ->(value, _error) { "CUSTOM: #{value}" }
        options = described_class.new(
          {
            escape: true,
            hidden: nil,
        ...
        aggregate_failures do
          ...
          expect(options.hidden_start).to eq(5)
          expect(options.hidden_end).to eq(10)
          expect(options.dot_key).to eq('.')
          expect(options.dash_key).to eq('~')
          expect(options.on_fail).to equal(on_fail)
        end
      end
    end
    ...
    context 'when called with override parameters' do
      ...
      it 'lets keyword arguments override positional layers' do
        options = described_class.new({ hidden_key: '#' }, hidden_key: 'X')
  ...
  describe '#set' do
    context 'when updating with a partial hash' do
      it 'keeps current values for unresolved keys' do
        options = described_class.new(hidden: true, hidden_key: '#', dash_key: '_')
        options.set(dot_key: ' ')
    ...
    context 'when updating with nil values' do
      it 'ignores nil and keeps the current value' do
        options = described_class.new(hidden_key: '#')
        options.set(hidden_key: nil)

        expect(options.hidden_key).to eq('#')
      end
    end

    context 'when updating with layered overrides' do
      it 'folds left-to-right then applies keywords' do
        options = described_class.new(hidden_key: '*')
      ...
      it 'raises TypeMismatchError' do
        ...
        expect { options.hidden = nil }
          .to raise_error(
            ...
            'CPF formatting option "hidden" must be of type boolean. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.hidden_key = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "hidden_key" must be of type string. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a non-string value' do
      it 'raises TypeMismatchError for an object' do
        ...
        expect { options.hidden_key = { not: 'a string' } }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "hidden_key" must be of type string. Got hash.'
          )
      end
  ...
  describe '#hidden_start=' do
    ...
    context 'when setting to an invalid range' do
      ...
      it 'raises OutOfRangeError for 11' do
        options = described_class.new

        expect { options.hidden_start = 11 }
          .to raise_error(
            CpfFmt::OutOfRangeError,
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.hidden_start = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "hidden_start" must be of type integer. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a non-integer value' do
      it 'raises TypeMismatchError for an object' do
        ...
        expect { options.hidden_start = { not: 'a number' } }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "hidden_start" must be of type integer. Got hash.'
          )
      end
  ...
  describe '#hidden_end=' do
    context 'when setting to a number value' do
      it 'sets hidden_end to the provided value' do
        options = described_class.new(hidden_end: 10)
        options.hidden_end = 9

        expect(options.hidden_end).to eq(9)
      end
    end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.hidden_end = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "hidden_end" must be of type integer. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a non-integer value' do
      it 'raises TypeMismatchError for an object' do
        ...
        expect { options.hidden_end = { not: 'a number' } }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "hidden_end" must be of type integer. Got hash.'
          )
      end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.dot_key = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "dot_key" must be of type string. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a non-string value' do
      it 'raises TypeMismatchError for an object' do
        ...
        expect { options.dot_key = { not: 'a string' } }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "dot_key" must be of type string. Got hash.'
          )
      end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.dash_key = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "dash_key" must be of type string. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a non-string value' do
      it 'raises TypeMismatchError for an object' do
        ...
        expect { options.dash_key = { not: 'a string' } }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "dash_key" must be of type string. Got hash.'
          )
      end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.escape = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "escape" must be of type boolean. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.encode = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "encode" must be of type boolean. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a nil value' do
      it 'raises TypeMismatchError' do
        ...
        expect { options.on_fail = nil }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "on_fail" must be of type function. Got nil.'
          )
      end
    end
    ...
    context 'when setting to a non-callable value' do
      it 'raises TypeMismatchError for an object' do
        ...
        expect { options.on_fail = { not: 'a function' } }
          .to raise_error(
            CpfFmt::TypeMismatchError,
            'CPF formatting option "on_fail" must be of type function. Got hash.'
          )
      end
  ...
  describe '#set_hidden_range' do
    context 'when called with valid values' do
      ...
      context 'when hidden_start equals hidden_end' do
        ...
        it 'accepts 10 for both ends' do
          options = described_class.new
          options.set_hidden_range(10, 10)

          aggregate_failures do
            expect(options.hidden_start).to eq(10)
            expect(options.hidden_end).to eq(10)
          end
        end
      end
    ...
    context 'when called with nil values' do
      ...
      context 'when hidden_start is nil' do
        it 'raises TypeMismatchError' do
          ...
          expect { options.set_hidden_range(nil, 10) }
            .to raise_error(
              CpfFmt::TypeMismatchError,
              'CPF formatting option "hidden_start" must be of type integer. Got nil.'
            )
    ...
    context 'when called with invalid values' do
      context 'when hidden_start is out of range' do
        it 'raises OutOfRangeError for -1' do
          ...
          expect { options.set_hidden_range(-1, 10) }
            .to raise_error(
              CpfFmt::OutOfRangeError,
              'CPF formatting option "hidden_start" must be an integer between 0 and 10. Got -1.'
            )
        ...
        it 'raises OutOfRangeError for 11' do
          ...
          expect { options.set_hidden_range(11, 10) }
            .to raise_error(
              CpfFmt::OutOfRangeError,
              'CPF formatting option "hidden_start" must be an integer between 0 and 10. Got 11.'
            )
      ...
      context 'when hidden_end is out of range' do
        ...
        it 'raises OutOfRangeError for 11' do
          ...
          expect { options.set_hidden_range(0, 11) }
            .to raise_error(
              CpfFmt::OutOfRangeError,
              'CPF formatting option "hidden_end" must be an integer between 0 and 10. Got 11.'
            )
      ...
      context 'when hidden_start is not an integer' do
        it 'raises TypeMismatchError for an object' do
          ...
          expect { options.set_hidden_range({ not: 'a number' }, 10) }
            .to raise_error(
              CpfFmt::TypeMismatchError,
              'CPF formatting option "hidden_start" must be of type integer. Got hash.'
            )
        ...
        it 'raises TypeMismatchError for a boolean' do
          ...
          expect { options.set_hidden_range(true, 10) }
            .to raise_error(
              CpfFmt::TypeMismatchError,
              'CPF formatting option "hidden_start" must be of type integer. Got boolean.'
            )
        ...
        it 'raises TypeMismatchError for a float' do
          ...
          expect { options.set_hidden_range(1.5, 10) }
            .to raise_error(
              CpfFmt::TypeMismatchError,
              'CPF formatting option "hidden_start" must be of type integer. Got float number.'
            )
